<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-date-picker class="filter-item" v-model="query.yearDate" @change="loadData" type="year" placeholder="选择月"></el-date-picker>
      <el-button class="filter-item" type="primary" icon="el-icon-search" @click="loadData">查询</el-button>
    </div>
    <el-row :gutter="32">
      <el-col :xs="24" :sm="24" :lg="24">
        <div class="chart-wrapper">
          <chart title="计划投资占比" ref="chart" :onclick="handleChartClick" height="calc(100vh - 230px)"></chart>
        </div>
      </el-col>
    </el-row>
  </div>
...
<script>
import Chart from '@/components/Chart/AmountPieChart'
import { getCompanyPlanStats } from '@/api/progress'
import OrganizationUnitTreeSelect from '@/components/OrganizationUnitTreeSelect'
// import { goCompanyTypeChart } from './utils'
import moment from 'moment'

export default {
  name: 'company-progress-plan-piechart',
  data() {
    return {
      query: { yearDate: null },
      chartData: {}
  ...
  methods: {
    ...
    loadData() {
      ...
      getCompanyPlanStats(this.query).then(res => {
