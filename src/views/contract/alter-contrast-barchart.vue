    ...
    <el-row :gutter="32">
      <el-col :xs="24" :sm="24" :lg="24">
        <div class="chart-wrapper">
          <chart title="合同与变更对比" ref="chart" :onclick="handleBarChartClick" height="calc(100vh - 230px)"></chart>
        </div>
      </el-col>
    </el-row>
  </div>
...
import { getAlterContrast } from '@/api/contract'
...
import * as alterationUtils from '@/views/alteration/utils'
...
export default {
  ...
  methods: {
    ...
    handleBarChartClick(params) {
      ...
      if (params.name === '变更') {
        const title = '变更列表'
        alterationUtils.goToList(this.$store, this.$router, title, query)
      }
    },
    loadData() {
      this.$refs.chart.showLoading()
      getAlterContrast(this.query).then(res => {
