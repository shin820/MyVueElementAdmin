    ...
    <el-row :gutter="32">
      <el-col :xs="24" :sm="24" :lg="24">
        <div class="chart-wrapper">
          <chart :showPercent=true title="变更等级分析" ref="chart" :onclick="handleBarChartClick" height="calc(100vh - 230px)"></chart>
        </div>
      </el-col>
    </el-row>
  </div>
...
import { getLevelAmounts } from '@/api/alteration'
...
export default {
  ...
  methods: {
    ...
    handleBarChartClick(params) {
      ...
      query.level = params.name
      goToCompanyChart(this.$store, this.$router, title, query)
    },
    loadData() {
      this.$refs.chart.showLoading()
      getLevelAmounts(this.query).then(res => {
