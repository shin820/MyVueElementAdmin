<template>
  <div class="app-container data-list-container">
    <div class="filter-container">
      ...
      <el-select clearable style="width: 120px" class="filter-item" @change="handleFilter" v-model="listQuery.statusName" placeholder="整改状态">
        <el-option key="已整改" label="已整改" value="已整改"></el-option>
        <el-option key="在整改" label="在整改" value="在整改"></el-option>
        <el-option key="逾期未整改" label="逾期未整改" value="逾期未整改"></el-option>
      </el-select>
      <el-select clearable style="width: 120px" class="filter-item" @change="handleFilter" v-model="listQuery.problemGrade" placeholder="问题等级">
        <el-option key="轻微" label="轻微" value="轻微"></el-option>
        <el-option key="一般" label="一般" value="一般"></el-option>
        <el-option key="较大" label="较大" value="较大"></el-option>
...
<script>
import { getPagedIssueList } from '@/api/inspection'
  ...
  methods: {
    getQueryFromUrl() {
      if (this.$route.query.organizationId && this.$route.query.organizationType) {
        ...
        this.listQuery.problemGrade = this.$route.query.problemGrade
        this.listQuery.from = this.$route.query.from
        this.listQuery.to = this.$route.query.to
        this.listQuery.statusName = this.$route.query.statusName
